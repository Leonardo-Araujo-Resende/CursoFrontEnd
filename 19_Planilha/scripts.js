...
function adicionarCompra(titulo, lugar, valorEsquerda, valorDireita, arrayAlimentos = null, quemPagou,  dataCompra, dataTransferencia = "Aguardando transferência", save = true, tarefaComItem = false){
    if(!(titulo && lugar && valorEsquerda && valorDireita && quemPagou !== "nenhum")){
        ...
        `<div class="auxjs">
            ...
            <div class="auxiliar-display">
                ...
                <div class="notinha">
                    <img src="img/download (4).jpeg" alt="Notinha" id="notinha">
                </div>
            </div>
        </div>`;
        ...
        if(arrayAlimentos.length != 0 && tarefaComItem == false){
            ...
            //Salva Local Storage
            if(save) salvaCompraLocalStorage({titulo: titulo, lugar: lugar, valorEsquerda: valorEsquerda, valorDireita: valorDireita, arrayAlimentos: arrayAlimentos, quemPagou: quemPagou,  dataCompra:dataFormatada, dataTransferencia: dataTransferencia})
...
butaoFinalizarAlimentosQuemComprou.addEventListener("click",()=>{
    ...
    adicionarCompra(inputTituloCompra.value, inputLugarCompra.value, parseFloat(valorPagarPelaCompraLeonardo.textContent).toFixed(3), parseFloat(valorPagarPelaCompraIgor.textContent).toFixed(3), null, inputQuemCompra.value, retornaDataHora(), dataTransferencia = "Aguardando transferência", save = true, tarefaComItem = true);
...
buttonAdicionarCompra.addEventListener("click", ()=>{
    resetarFormularioQuemComprou()
    adicionarCompra(inputTituloCompra.value, inputLugarCompra.value, (inputValorCompra.value/2).toFixed(3), (inputValorCompra.value/2).toFixed(3), null, inputQuemCompra.value, retornaDataHora(), dataTransferencia = "Aguardando transferência", save = true, tarefaComItem = false);
...
function carregaCompras(){
    ...
    compras.forEach(compra => {
        adicionarCompra(compra.titulo, compra.lugar, compra.valorEsquerda, compra.valorDireita, compra.arrayAlimentos, compra.quemPagou, compra.dataCompra, compra.dataTransferencia, false, true);
    })
